...
fn to_hashes(wordlist: &[String], sublens: u32) -> HashMap<String, u32> {
    ...
    let iter = wordlist
        .iter()
        .filter(|k| k.chars().count() > 0 && !k.contains('\'') && !all_lowercase(k));
    ...
    let iter = iter.flat_map(|k| {
        ...
        let chvec: Vec<String> = fullchars
            .windows(sublens as usize)
            .map(|chars| chars.iter().clone().collect::<String>())
            .collect();
...
impl WordBuilder {
    ...
    fn word(&mut self) -> Option<String> {
        ...
        loop {
            ...
            let mut endsum = 0u32;
            let possibilities: Vec<(&str, u32)> = self.subs
                .iter()
                .filter_map(|(k, v)| {
